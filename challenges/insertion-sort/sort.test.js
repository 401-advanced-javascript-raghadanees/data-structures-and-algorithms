...
describe('sort tests', () => {
  ...
  it('throws errors with non-arrays', () => {
    ...
    expect(response1).toThrow('Invalid sequence');
    let str = 'test';
    let response2 = () => {
      insertionSort(str);
    };
    expect(response2).toThrow('Invalid sequence');
    // expect(response).toEqual('Invalid sequence');
  });
